...
vec2 curve(vec2 uv)
{
  ...
  uv.y *= 1.0 + pow((abs(uv.x) / 16.0), 2.0);
  uv  = (uv / 2.0) + 0.5;
  uv =  uv * 0.92 + 0.04;
  return uv;
}
...
void main()
{
  ...
  col.r = texture2D(uSampler, vec2(x + uv.x + 0.0005, uv.y + 0.0005)).x + 0.05;
  col.g = texture2D(uSampler, vec2(x + uv.x + 0.0000, uv.y - 0.0005)).y + 0.05;
  col.b = texture2D(uSampler, vec2(x + uv.x - 0.0005, uv.y + 0.0000)).z + 0.05;
  ...
  float scans = clamp(0.35 + 0.35 * sin(1.25 * time + uv.y * 1.5), 0.0, 1.0);

  float s = pow(scans, 1.7);
  col = col * vec3(0.6 + 0.5 * s);
